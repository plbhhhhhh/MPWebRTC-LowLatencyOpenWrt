...
int PASCAL wWinMain(HINSTANCE instance,
  ...
  if (!ApplyWebrtcStackModeFromFlags()) {
    return -1;
  }

  std::string force_fieldtrials = absl::GetFlag(FLAGS_force_fieldtrials);
  constexpr char kDisableAutoResizeTrial[] =
      "WebRTC-Video-DisableAutomaticResize/Enabled/";
  if (force_fieldtrials.find("WebRTC-Video-DisableAutomaticResize/") ==
      std::string::npos) {
    if (!force_fieldtrials.empty() && force_fieldtrials.back() != '/') {
      force_fieldtrials.push_back('/');
    }
    force_fieldtrials += kDisableAutoResizeTrial;
  }
  webrtc::Environment env = webrtc::CreateEnvironment(
      std::make_unique<webrtc::FieldTrials>(force_fieldtrials));
