...
constexpr uint64_t kLateAckLogInterval = 500;

struct ProbeClusterAggregate {
  Timestamp first_receive_time = Timestamp::PlusInfinity();
  Timestamp last_receive_time = Timestamp::MinusInfinity();
  DataSize total_size = DataSize::Zero();
  int packet_count = 0;
};

void MaybePublishProbeBitrate(
    const std::map<std::pair<uint16_t, int>, ProbeClusterAggregate>&
        probe_aggregates) {
  if (!IsWebrtcMultipathExtensionEnabled()) {
    return;
  }
  for (const auto& [key, aggregate] : probe_aggregates) {
    const TimeDelta duration =
        aggregate.last_receive_time - aggregate.first_receive_time;
    if (aggregate.packet_count < 2 || !duration.IsFinite() ||
        duration <= TimeDelta::Zero()) {
      continue;
    }
    const double bitrate_bps =
        static_cast<double>(aggregate.total_size.bytes()) * 8.0 /
        duration.seconds<double>();
    if (bitrate_bps <= 0.0) {
      continue;
    }
    const uint32_t clamped_bps = static_cast<uint32_t>(
        std::min<double>(bitrate_bps, std::numeric_limits<uint32_t>::max()));
    UpdateProbeBitrateForNetwork(key.first, clamped_bps);
  }
}
...
void TransportFeedbackAdapter::AddPacket(const RtpPacketToSend& packet_to_send,
  ...
  feedback.path_hint = packet_to_send.path_hint();
...
std::optional<SentPacket> TransportFeedbackAdapter::ProcessSentPacket(
  ...
  if (sent_packet.info.included_in_feedback || sent_packet.packet_id != -1) {
    ...
    if (it != history_.end()) {
      if (sent_packet.info.network_id.has_value()) {
        const RouteEndpoint& local = it->second.network_route.local;
        it->second.network_route.local =
            RouteEndpoint(local.adapter_type(), local.adapter_id(),
                          *sent_packet.info.network_id, local.uses_turn());
      }
...
TransportFeedbackAdapter::ProcessTransportFeedback(
  ...
  std::map<std::pair<uint16_t, int>, ProbeClusterAggregate> probe_aggregates;
  ...
  feedback.ForAllPackets([&](uint16_t sequence_number,
    ...
    if (delta_since_base.IsFinite()) {
      result.receive_time = current_offset_ +
                            delta_since_base.RoundDownTo(TimeDelta::Millis(1));
      const int probe_id = packet_feedback->sent.pacing_info.probe_cluster_id;
      if (probe_id != PacedPacketInfo::kNotAProbe) {
        const uint16_t network_id = packet_feedback->network_route.local.network_id();
        auto& aggregate = probe_aggregates[{network_id, probe_id}];
        aggregate.first_receive_time =
            std::min(aggregate.first_receive_time, result.receive_time);
        aggregate.last_receive_time =
            std::max(aggregate.last_receive_time, result.receive_time);
        aggregate.total_size += packet_feedback->sent.size;
        ++aggregate.packet_count;
      }
    }
    result.send_path_network_id =
        packet_feedback->network_route.local.network_id();
    result.rtp_packet_info = {.ssrc = packet_feedback->ssrc,
                              .rtp_sequence_number =
                                  packet_feedback->rtp_sequence_number,
                              .is_retransmission =
                                  packet_feedback->is_retransmission,
                              .path_hint = packet_feedback->path_hint};
  ...
  MaybePublishProbeBitrate(probe_aggregates);
...
std::optional<PacketFeedback> TransportFeedbackAdapter::RetrievePacketFeedback(
  ...
  if (it == history_.end()) {
    // Suppressed noisy warning for expired history lookup.
    return std::nullopt;
  }
  ...
  if (!received) {
    if (!it->second.was_reported_lost) {
      it->second.was_reported_lost = true;
      ++reported_lost_packets_;
    }
  } else if (it->second.was_reported_lost) {
    ++late_acked_packets_;
    if (late_acked_packets_ % kLateAckLogInterval == 0) {
      const double late_ack_ratio =
          reported_lost_packets_ > 0
              ? static_cast<double>(late_acked_packets_) /
                    static_cast<double>(reported_lost_packets_)
              : 0.0;
      RTC_LOG(LS_INFO) << "TWCC LateAck: late_acked=" << late_acked_packets_
                       << ", reported_lost=" << reported_lost_packets_
                       << ", ratio=" << (late_ack_ratio * 100.0) << "%";
    }
  }
  if (received) {
