...
struct PacketFeedback {
  ...
  int path_hint = -1;
  // Set when a packet has been reported as not-received in feedback at least
  // once and is still kept in history waiting for a potential late ACK.
  bool was_reported_lost = false;
};
...
class TransportFeedbackAdapter {
 ...
 private:
  ...
  uint64_t reported_lost_packets_ = 0;
  uint64_t late_acked_packets_ = 0;
};
