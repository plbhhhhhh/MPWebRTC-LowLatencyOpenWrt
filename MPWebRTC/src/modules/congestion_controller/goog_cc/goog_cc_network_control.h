...
class GoogCcNetworkController : public NetworkControllerInterface {
 ...
 private:
  ...
  NetworkControlUpdate OnTransportPacketsFeedbackMultipath(
      TransportPacketsFeedback report);

  struct MultipathPerPathBweState {
    std::unique_ptr<DelayBasedBwe> delay_based_bwe;
    std::unique_ptr<ProbeBitrateEstimator> probe_bitrate_estimator;
    std::unique_ptr<AcknowledgedBitrateEstimatorInterface>
        acknowledged_bitrate_estimator;
  };

  MultipathPerPathBweState& GetOrCreateMultipathPerPathBwe(
      uint16_t network_id);
  ...
  std::map<uint16_t, std::unique_ptr<MultipathPerPathBweState>>
      multipath_per_path_bwes_;
  BandwidthUsage multipath_worst_delay_detector_state_ =
      BandwidthUsage::kBwNormal;
  TimeDelta multipath_max_expected_bwe_period_ = TimeDelta::Millis(100);
};
