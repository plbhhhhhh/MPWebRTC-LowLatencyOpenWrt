...
std::vector<ProbeClusterConfig> ProbeController::RequestProbeOnRouteChange(
    Timestamp at_time) {
  // Route-change probing should always produce a fresh cluster so that newly
  // selected paths can quickly get a probe-based estimate.
  DataRate base_rate = estimated_bitrate_;
  if (base_rate.IsZero()) {
    base_rate = start_bitrate_;
  }
  if (base_rate.IsZero()) {
    base_rate = max_total_allocated_bitrate_;
  }
  if (base_rate.IsZero()) {
    base_rate = DataRate::KilobitsPerSec(300);
  }
  return InitiateProbing(at_time, {base_rate}, false);
}
