...
class ProbeController {
 public:
  ...
  ABSL_MUST_USE_RESULT std::vector<ProbeClusterConfig> RequestProbe(
      Timestamp at_time);
  // Force one probe cluster for route-change validation. Unlike RequestProbe(),
  // this does not depend on ALR/drop-recovery conditions.
  ABSL_MUST_USE_RESULT std::vector<ProbeClusterConfig>
  RequestProbeOnRouteChange(Timestamp at_time);
