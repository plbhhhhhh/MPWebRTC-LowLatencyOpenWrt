...
// Extended multipath with larger playout delay can tolerate temporary path
// asymmetry/reordering. Keep probe backoff milder to avoid unnecessary target
// bitrate collapse.
constexpr double kExtendedProbeDropThroughputFraction = 0.95;
// In extended mode, keep a modest floor to acknowledged throughput when delay
// detector does not indicate global overuse.
constexpr double kExtendedAckOptimisticFloorFraction = 0.92;
// In extended mode, late feedback from higher-delay path can be reported as
// temporary loss. Ignore very recent "lost" packets to reduce false loss.
constexpr TimeDelta kExtendedLossGraceWindow = TimeDelta::Millis(120);
constexpr uint16_t kSyntheticPrimaryPathNetworkId = 0xFFFE;
constexpr uint16_t kSyntheticSecondaryPathNetworkId = 0xFFFD;
...
BandwidthUsage WorstBandwidthUsage(BandwidthUsage a, BandwidthUsage b) {
  return static_cast<BandwidthUsage>(
      std::max(static_cast<int>(a), static_cast<int>(b)));
}

bool IsExtendedMultipathMode() {
  return GetWebrtcMultipathSendMode() == WebrtcMultipathSendMode::kExtended;
}

std::map<uint16_t, std::vector<PacketResult>>
PartitionFeedbackBySendPathNetworkId(const TransportPacketsFeedback& report) {
  std::map<uint16_t, std::vector<PacketResult>> partitions;
  for (const PacketResult& p : report.packet_feedbacks) {
    uint16_t id = p.send_path_network_id.value_or(0);
    if (id == 0 && p.rtp_packet_info.has_value()) {
      if (p.rtp_packet_info->path_hint == 0) {
        id = kSyntheticPrimaryPathNetworkId;
      } else if (p.rtp_packet_info->path_hint == 1) {
        id = kSyntheticSecondaryPathNetworkId;
      }
    }
    partitions[id].push_back(p);
  }
  return partitions;
}
...
NetworkControlUpdate GoogCcNetworkController::OnNetworkRouteChange(
  ...
  multipath_per_path_bwes_.clear();
  ClearDelayBasedBitrateMap();
  multipath_worst_delay_detector_state_ = BandwidthUsage::kBwNormal;
  multipath_max_expected_bwe_period_ = TimeDelta::Millis(100);
...
NetworkControlUpdate GoogCcNetworkController::OnProcessInterval(
  ...
  update.probe_cluster_configs.insert(update.probe_cluster_configs.end(),
                                      probes.begin(), probes.end());
  if (ConsumeGccProbeRequest()) {
    auto forced_probes =
        probe_controller_->RequestProbeOnRouteChange(msg.at_time);
    update.probe_cluster_configs.insert(update.probe_cluster_configs.end(),
                                        forced_probes.begin(),
                                        forced_probes.end());
  }
...
NetworkControlUpdate GoogCcNetworkController::OnRoundTripTimeUpdate(
  ...
  if (IsWebrtcMultipathExtensionEnabled()) {
    for (auto& kv : multipath_per_path_bwes_) {
      if (kv.second && kv.second->delay_based_bwe) {
        kv.second->delay_based_bwe->OnRttUpdate(msg.round_trip_time);
      }
    }
  }
  ...
  if (msg.min_total_allocated_bitrate &&
    ...
    if (use_min_allocatable_as_lower_bound_) {
      ...
      if (IsWebrtcMultipathExtensionEnabled()) {
        for (auto& kv : multipath_per_path_bwes_) {
          if (kv.second && kv.second->delay_based_bwe) {
            kv.second->delay_based_bwe->SetMinBitrate(min_data_rate_);
          }
        }
      }
...
std::vector<ProbeClusterConfig> GoogCcNetworkController::ResetConstraints(
  ...
  if (IsWebrtcMultipathExtensionEnabled()) {
    for (auto& kv : multipath_per_path_bwes_) {
      if (kv.second && kv.second->delay_based_bwe) {
        if (starting_rate_)
          kv.second->delay_based_bwe->SetStartBitrate(*starting_rate_);
        kv.second->delay_based_bwe->SetMinBitrate(min_data_rate_);
      }
    }
  }
...
NetworkControlUpdate GoogCcNetworkController::OnTransportPacketsFeedback(
  ...
  if (IsWebrtcMultipathExtensionEnabled()) {
    return OnTransportPacketsFeedbackMultipath(std::move(report));
  }
  ...
  if (previously_in_alr_ && !alr_start_time.has_value()) {
    ...
    for (auto& kv : multipath_per_path_bwes_) {
      if (kv.second && kv.second->acknowledged_bitrate_estimator) {
        kv.second->acknowledged_bitrate_estimator->SetAlrEndedTime(
...
NetworkControlUpdate GoogCcNetworkController::GetNetworkState(
  ...
  update.target_rate->network_estimate.bwe_period =
      IsWebrtcMultipathExtensionEnabled()
          ? multipath_max_expected_bwe_period_
...
#pragma clang diagnostic pop
    ...
    BandwidthUsage delay_state_for_probe =
        IsWebrtcMultipathExtensionEnabled()
            ? multipath_worst_delay_detector_state_
...
GoogCcNetworkController::MultipathPerPathBweState&
GoogCcNetworkController::GetOrCreateMultipathPerPathBwe(uint16_t network_id) {
  auto it = multipath_per_path_bwes_.find(network_id);
  if (it != multipath_per_path_bwes_.end()) {
    return *it->second;
  }
  auto state = std::make_unique<MultipathPerPathBweState>();
  state->delay_based_bwe = std::make_unique<DelayBasedBwe>(
      &env_.field_trials(), &env_.event_log(), network_state_predictor_.get());
  state->delay_based_bwe->SetMinBitrate(min_data_rate_);
  if (starting_rate_) {
    state->delay_based_bwe->SetStartBitrate(*starting_rate_);
  }
  state->probe_bitrate_estimator =
      std::make_unique<ProbeBitrateEstimator>(&env_.event_log());
  state->acknowledged_bitrate_estimator =
      AcknowledgedBitrateEstimatorInterface::Create(&env_.field_trials());
  MultipathPerPathBweState* ptr = state.get();
  multipath_per_path_bwes_[network_id] = std::move(state);
  return *ptr;
}

NetworkControlUpdate
GoogCcNetworkController::OnTransportPacketsFeedbackMultipath(
    TransportPacketsFeedback report) {
  if (congestion_window_pushback_controller_) {
    congestion_window_pushback_controller_->UpdateOutstandingData(
        report.data_in_flight.bytes());
  ...
  if (IsExtendedMultipathMode()) {
    std::vector<PacketResult> filtered_feedbacks;
    filtered_feedbacks.reserve(report.packet_feedbacks.size());
    size_t skipped_recent_losses = 0;
    for (const PacketResult& packet : report.packet_feedbacks) {
      if (!packet.IsReceived() && packet.sent_packet.send_time.IsFinite() &&
          report.feedback_time - packet.sent_packet.send_time <
              kExtendedLossGraceWindow) {
        ++skipped_recent_losses;
        continue;
      }
      filtered_feedbacks.push_back(packet);
    }
    if (skipped_recent_losses > 0) {
      RTC_LOG(LS_INFO) << "MP-GCC filtered likely reordered losses: "
                       << skipped_recent_losses;
    }
    report.packet_feedbacks = std::move(filtered_feedbacks);
    if (report.packet_feedbacks.empty()) {
      return NetworkControlUpdate();
    }
  }

  auto partitions = PartitionFeedbackBySendPathNetworkId(report);
  static uint32_t s_multipath_feedback_reports = 0;
  ++s_multipath_feedback_reports;
  if (s_multipath_feedback_reports % 200 == 0) {
    size_t non_zero_network_ids = 0;
    for (const auto& kv : partitions) {
      if (kv.first != 0) {
        ++non_zero_network_ids;
      }
    }
    RTC_LOG(LS_INFO) << "MP-GCC FeedbackPartitions: total="
                     << partitions.size()
                     << ", non_zero_network_ids=" << non_zero_network_ids;
  }
  if (partitions.size() <= 1) {
    RTC_LOG(LS_WARNING)
        << "MP-GCC only sees <=1 send-path partition in feedback; per-path "
           "GCC may not be effective (network_id collapse or missing "
           "network_id in sent packet info).";
  }

  std::optional<DataRate> aggregated_probe_for_loss;
  multipath_worst_delay_detector_state_ = BandwidthUsage::kBwNormal;
  bool first_bwe_period = true;

  bool any_updated = false;
  bool any_probe_set_send_bitrate = false;
  bool any_recovered_overuse = false;
  DataRate aggregated_delay_based_estimate = DataRate::Zero();
  DataRate aggregated_probe_target = DataRate::Zero();
  int overusing_paths = 0;
  int underusing_paths = 0;
  int normal_paths = 0;

  for (auto& entry : partitions) {
    const uint16_t network_id = entry.first;
    std::vector<PacketResult>& packets = entry.second;

    MultipathPerPathBweState& path = GetOrCreateMultipathPerPathBwe(network_id);

    TransportPacketsFeedback sub_report;
    sub_report.feedback_time = report.feedback_time;
    sub_report.data_in_flight = report.data_in_flight;
    sub_report.transport_supports_ecn = report.transport_supports_ecn;
    sub_report.packet_feedbacks = std::move(packets);

    std::vector<PacketResult> sorted = sub_report.SortedByReceiveTime();
    path.acknowledged_bitrate_estimator->IncomingPacketFeedbackVector(sorted);
    std::optional<DataRate> path_acknowledged_bitrate =
        path.acknowledged_bitrate_estimator->bitrate();
    for (const auto& feedback : sorted) {
      if (feedback.sent_packet.pacing_info.probe_cluster_id !=
          PacedPacketInfo::kNotAProbe) {
        path.probe_bitrate_estimator->HandleProbeAndEstimateBitrate(feedback);
      }
    }

    std::optional<DataRate> probe_bitrate =
        path.probe_bitrate_estimator->FetchAndResetLastEstimatedBitrate();

    if (ignore_probes_lower_than_network_estimate_ && probe_bitrate &&
        estimate_ && *probe_bitrate < path.delay_based_bwe->last_estimate() &&
        *probe_bitrate < estimate_->link_capacity_lower) {
      probe_bitrate.reset();
    }
    if (limit_probes_lower_than_throughput_estimate_ && probe_bitrate &&
        path_acknowledged_bitrate) {
      const double probe_drop_fraction =
          IsExtendedMultipathMode() ? kExtendedProbeDropThroughputFraction
                                    : kProbeDropThroughputFraction;
      DataRate limit =
          std::min(path.delay_based_bwe->last_estimate(),
                   *path_acknowledged_bitrate * probe_drop_fraction);
      probe_bitrate = std::max(*probe_bitrate, limit);
    }

    DelayBasedBwe::Result result =
        path.delay_based_bwe->IncomingPacketFeedbackVector(
            sub_report, path_acknowledged_bitrate, probe_bitrate, estimate_,
            alr_start_time.has_value());

    const int64_t est_bps_i64 = path.delay_based_bwe->last_estimate().bps();
    const uint32_t est_bps = static_cast<uint32_t>(std::max<int64_t>(
        0, std::min(est_bps_i64,
                    static_cast<int64_t>(std::numeric_limits<uint32_t>::max()))));
    UpdateDelayBasedBitrateForNetwork(network_id, est_bps);
    aggregated_delay_based_estimate += path.delay_based_bwe->last_estimate();

    multipath_worst_delay_detector_state_ = WorstBandwidthUsage(
        multipath_worst_delay_detector_state_, result.delay_detector_state);
    if (result.delay_detector_state == BandwidthUsage::kBwOverusing) {
      ++overusing_paths;
    } else if (result.delay_detector_state == BandwidthUsage::kBwUnderusing) {
      ++underusing_paths;
    } else {
      ++normal_paths;
    }

    TimeDelta period = path.delay_based_bwe->GetExpectedBwePeriod();
    if (first_bwe_period) {
      multipath_max_expected_bwe_period_ = period;
      first_bwe_period = false;
    } else {
      multipath_max_expected_bwe_period_ =
          std::max(multipath_max_expected_bwe_period_, period);
    }

    if (probe_bitrate.has_value()) {
      aggregated_probe_for_loss =
          aggregated_probe_for_loss
              ? (*aggregated_probe_for_loss + *probe_bitrate)
              : probe_bitrate;
    }

    any_updated |= result.updated;
    any_recovered_overuse |= result.recovered_from_overuse;

    if (result.updated) {
      if (result.probe) {
        any_probe_set_send_bitrate = true;
        aggregated_probe_target += result.target_bitrate;
      }
    }
  }

  NetworkControlUpdate update;
  BandwidthUsage multipath_delay_state_for_loss =
      multipath_worst_delay_detector_state_;
  if (IsExtendedMultipathMode() && !partitions.empty()) {
    // In extended mode, avoid letting a single lagging path dominate global
    // loss-based reaction when the other path is healthy.
    if (overusing_paths == 0) {
      multipath_delay_state_for_loss =
          underusing_paths > 0 ? BandwidthUsage::kBwUnderusing
                               : BandwidthUsage::kBwNormal;
    } else if (overusing_paths < static_cast<int>(partitions.size()) &&
               normal_paths > 0) {
      multipath_delay_state_for_loss = BandwidthUsage::kBwNormal;
    } else {
      multipath_delay_state_for_loss = BandwidthUsage::kBwOverusing;
    }
    multipath_worst_delay_detector_state_ = multipath_delay_state_for_loss;
  }

  if (any_updated) {
    DataRate clamped_aggregated_delay_target =
        std::min(max_data_rate_,
                 std::max(min_data_rate_, aggregated_delay_based_estimate));
    if (IsExtendedMultipathMode() && acknowledged_bitrate &&
        multipath_delay_state_for_loss != BandwidthUsage::kBwOverusing) {
      const DataRate optimistic_floor = std::min(
          max_data_rate_,
          std::max(min_data_rate_,
                   *acknowledged_bitrate * kExtendedAckOptimisticFloorFraction));
      clamped_aggregated_delay_target =
          std::max(clamped_aggregated_delay_target, optimistic_floor);
    }
    if (any_probe_set_send_bitrate) {
      DataRate probe_target = aggregated_probe_target.IsZero()
                                  ? clamped_aggregated_delay_target
                                  : std::min(max_data_rate_,
                                             std::max(min_data_rate_,
                                                      aggregated_probe_target));
      bandwidth_estimation_->SetSendBitrate(probe_target, report.feedback_time);
    }
    bandwidth_estimation_->UpdateDelayBasedEstimate(
        report.feedback_time, clamped_aggregated_delay_target);
  }

  bandwidth_estimation_->UpdateLossBasedEstimator(
      report, multipath_delay_state_for_loss, aggregated_probe_for_loss,
      alr_start_time.has_value());
