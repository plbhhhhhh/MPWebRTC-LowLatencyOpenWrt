...
constexpr uint32_t kBatchDurationLogIntervalBatches = 200;
constexpr uint32_t kNonBatchedLogIntervalPackets = 200;
constexpr int kMinSchedulingBatchDurationMs = 10;
constexpr int kMaxSchedulingBatchDurationMs = 20;
...
bool IsH264ParameterSetNaluType(uint8_t nalu_type) {
  return nalu_type == 7 || nalu_type == 8;
}

bool HasH264ParameterSetNaluInStapA(ArrayView<const uint8_t> payload) {
  // STAP-A payload: [STAP-A NAL header][2-byte len][NALU]...
  if (payload.size() < 3) {
    return false;
  }
  size_t offset = 1;
  while (offset + 2 <= payload.size()) {
    const size_t nalu_size =
        (static_cast<size_t>(payload[offset]) << 8) | payload[offset + 1];
    offset += 2;
    if (nalu_size == 0 || offset + nalu_size > payload.size()) {
      return false;
    }
    const uint8_t nalu_type = payload[offset] & 0x1F;
    if (IsH264ParameterSetNaluType(nalu_type)) {
      return true;
    }
    offset += nalu_size;
  }
...
bool IsH264ParameterSetPacket(const RtpPacketToSend& packet) {
  if (packet.packet_type() != RtpPacketMediaType::kVideo || !packet.is_h264()) {
    return false;
  }
  ...
  const uint8_t nalu_type = payload[0] & 0x1F;
  if (IsH264ParameterSetNaluType(nalu_type)) {
    return true;
  }
  ...
  // STAP-A.
  if (nalu_type == 24) {
    return HasH264ParameterSetNaluInStapA(payload);
  }

  // FU-A: check reconstructed original NALU type in FU header.
  if (nalu_type == 28 && payload.size() >= 2) {
    const uint8_t original_nalu_type = payload[1] & 0x1F;
    return IsH264ParameterSetNaluType(original_nalu_type);
  }

  return false;
}

int DeterminePacketPriority(const RtpPacketToSend& packet) {
  if (packet.packet_type() == RtpPacketMediaType::kRetransmission) {
    return kPacketPriorityRetransmission;
  }
  if (packet.packet_type() == RtpPacketMediaType::kAudio) {
    return kPacketPriorityAudio;
  }
  if (packet.is_key_frame() || IsH264ParameterSetPacket(packet)) {
    return kPacketPriorityKeyFrame;
  }
  if (packet.packet_type() == RtpPacketMediaType::kForwardErrorCorrection ||
    ...
    return kPacketPriorityRedundancy;
  }
  return kPacketPriorityOther;
}

int ClampSchedulingBatchDurationMs(int duration_ms) {
  return std::max(
      kMinSchedulingBatchDurationMs,
      std::min(kMaxSchedulingBatchDurationMs, duration_ms));
}
}  // namespace
...
RtpSenderEgress::RtpSenderEgress(const Environment& env,
    ...
    : env_(env),
      ...
      fec_generator_primary_(config.fec_generator),
      ...
      send_rates_primary_(kNumMediaTypes,
                          BitrateTracker(kBitrateStatisticsWindow)),
      send_rates_secondary_(kNumMediaTypes,
                            BitrateTracker(kBitrateStatisticsWindow)),
  ...
  RTC_DCHECK(config.transport_feedback_callback == nullptr)
      << "transport_feedback_callback is no longer used and will soon be "
         "deleted.";
  if (fec_generator_primary_) {
    fec_generator_secondary_owned_ = fec_generator_primary_->CloneForPathHint();
    fec_generator_secondary_ = fec_generator_secondary_owned_.get();
  }
  RTC_LOG(LS_INFO) << "RTP-EGRESS Init: ssrc=" << ssrc_
                   << ", is_audio=" << is_audio_
                   << ", batching_enabled=" << enable_send_packet_batching_
                   << ", path_hint_fec_split="
                   << (fec_generator_secondary_ != nullptr);
...
void RtpSenderEgress::SendPacket(std::unique_ptr<RtpPacketToSend> packet,
  ...
  if (need_rtp_packet_infos_ &&
    ...
    rtp_sequence_number_map_->InsertPacket(
        packet->SequenceNumber(),
        RtpSequenceNumberMap::Info(timestamp, packet->is_first_packet_of_frame(),
                                   packet->Marker(), packet->path_hint()));
  }

  VideoFecGenerator* selected_fec_generator = nullptr;
  if (packet->fec_protect_packet()) {
    selected_fec_generator = SelectFecGeneratorForPacket(*packet);
  }
  if (selected_fec_generator && packet->fec_protect_packet()) {
    ...
    std::optional<std::pair<FecProtectionParams, FecProtectionParams>>
        new_fec_params;
    if (selected_fec_generator == fec_generator_secondary_) {
      new_fec_params.swap(pending_fec_params_secondary_);
    } else {
      new_fec_params.swap(pending_fec_params_primary_);
    }
    if (new_fec_params) {
      selected_fec_generator->SetProtectionParameters(new_fec_params->first,
    ...
    if (packet->is_red()) {
      ...
      selected_fec_generator->AddPacketAndGenerateFec(unpacked_packet);
    } else {
      // If not RED encapsulated - we can just insert packet directly.
      selected_fec_generator->AddPacketAndGenerateFec(*packet);
    }
  }
  ...
  const int packet_priority = DeterminePacketPriority(*packet);
  auto compound_packet =
      Packet{std::move(packet), pacing_info, now, packet_priority};
  if (enable_send_packet_batching_ && !is_audio_) {
    packets_to_send_.push_back(std::move(compound_packet));
  } else {
    const int batch_duration_ms =
        ClampSchedulingBatchDurationMs(kMinSchedulingBatchDurationMs);
    ++batch_duration_log_non_batched_packets_;
    MaybeLogBatchingModeStats();
    CompleteSendPacket(
        compound_packet, true, true, batch_duration_ms);
  }
}
...
void RtpSenderEgress::OnBatchComplete() {
  ...
  Timestamp min_now = packets_to_send_.front().now;
  Timestamp max_now = packets_to_send_.front().now;
  for (const auto& packet : packets_to_send_) {
    min_now = std::min(min_now, packet.now);
    max_now = std::max(max_now, packet.now);
  }
  const int raw_batch_duration_ms = (max_now - min_now).ms();
  const int batch_duration_ms =
      ClampSchedulingBatchDurationMs(raw_batch_duration_ms);
  RecordBatchedDurationStats(raw_batch_duration_ms, batch_duration_ms,
                             packets_to_send_.size());
  MaybeLogBatchingModeStats();
  for (size_t i = 0; i < packets_to_send_.size(); ++i) {
    CompleteSendPacket(packets_to_send_[i], i == 0,
                       i + 1 == packets_to_send_.size(), batch_duration_ms);
  }
  packets_to_send_.clear();
}

void RtpSenderEgress::RecordBatchedDurationStats(int raw_batch_duration_ms,
                                                 int clamped_batch_duration_ms,
                                                 size_t packets_in_batch) {
  RTC_DCHECK_RUN_ON(worker_queue_);
  ++batch_duration_log_batched_batches_;
  batch_duration_log_batched_raw_sum_ms_ +=
      static_cast<uint64_t>(raw_batch_duration_ms);
  batch_duration_log_batched_clamped_sum_ms_ +=
      static_cast<uint64_t>(clamped_batch_duration_ms);
  batch_duration_log_batched_packets_ += static_cast<uint64_t>(packets_in_batch);
  if (batch_duration_log_batched_batches_ == 1) {
    batch_duration_log_batched_raw_min_ms_ = raw_batch_duration_ms;
    batch_duration_log_batched_raw_max_ms_ = raw_batch_duration_ms;
  } else {
    batch_duration_log_batched_raw_min_ms_ = std::min(
        batch_duration_log_batched_raw_min_ms_, raw_batch_duration_ms);
    batch_duration_log_batched_raw_max_ms_ = std::max(
        batch_duration_log_batched_raw_max_ms_, raw_batch_duration_ms);
  }
}

void RtpSenderEgress::MaybeLogBatchingModeStats() {
  RTC_DCHECK_RUN_ON(worker_queue_);
  if (batch_duration_log_batched_batches_ >= kBatchDurationLogIntervalBatches ||
      batch_duration_log_non_batched_packets_ >=
          kNonBatchedLogIntervalPackets) {
    if (batch_duration_log_batched_batches_ > 0) {
      const double avg_raw_ms =
          static_cast<double>(batch_duration_log_batched_raw_sum_ms_) /
          static_cast<double>(batch_duration_log_batched_batches_);
      const double avg_clamped_ms =
          static_cast<double>(batch_duration_log_batched_clamped_sum_ms_) /
          static_cast<double>(batch_duration_log_batched_batches_);
      const double avg_packets =
          static_cast<double>(batch_duration_log_batched_packets_) /
          static_cast<double>(batch_duration_log_batched_batches_);
      RTC_LOG(LS_INFO) << "RTP-EGRESS BatchedDuration: batched_batches="
                       << batch_duration_log_batched_batches_
                       << ", avg_raw_ms=" << avg_raw_ms
                       << ", min_raw_ms="
                       << batch_duration_log_batched_raw_min_ms_
                       << ", max_raw_ms="
                       << batch_duration_log_batched_raw_max_ms_
                       << ", avg_clamped_ms=" << avg_clamped_ms
                       << ", avg_packets_per_batch=" << avg_packets;
    }
    RTC_LOG(LS_INFO) << "RTP-EGRESS NonBatched: packets="
                     << batch_duration_log_non_batched_packets_;
    batch_duration_log_batched_batches_ = 0;
    batch_duration_log_batched_raw_sum_ms_ = 0;
    batch_duration_log_batched_clamped_sum_ms_ = 0;
    batch_duration_log_batched_packets_ = 0;
    batch_duration_log_batched_raw_min_ms_ = 0;
    batch_duration_log_batched_raw_max_ms_ = 0;
    batch_duration_log_non_batched_packets_ = 0;
  }
}
...
void RtpSenderEgress::CompleteSendPacket(const Packet& compound_packet,
  ...
  RTC_DCHECK_RUN_ON(worker_queue_);
  auto& [packet, pacing_info, now, packet_priority] = compound_packet;
  RTC_CHECK(packet);
  ...
  options.first_packet_in_batch = first_in_batch;
  options.last_packet_in_batch = last_in_batch;
  options.batch_duration_ms = batch_duration_ms;
  options.probe_cluster_id = pacing_info.probe_cluster_id;

  options.packet_priority = packet_priority;
  options.path_hint = packet->path_hint();
...
void RtpSenderEgress::SetFecProtectionParameters(
  ...
  RTC_DCHECK_RUN_ON(worker_queue_);
  pending_fec_params_primary_.emplace(delta_params, key_params);
  pending_fec_params_secondary_.emplace(delta_params, key_params);
}

void RtpSenderEgress::SetFecProtectionParametersForPathHint(
    int path_hint,
    const FecProtectionParams& delta_params,
    const FecProtectionParams& key_params) {
  RTC_DCHECK_RUN_ON(worker_queue_);
  if (NormalizePathHint(path_hint) == RtpPacketToSend::kSecondaryPathHint) {
    pending_fec_params_secondary_.emplace(delta_params, key_params);
  } else {
    pending_fec_params_primary_.emplace(delta_params, key_params);
  }
...
RtpSenderEgress::FetchFecPackets() {
  ...
  if (fec_generator_primary_) {
    std::vector<std::unique_ptr<RtpPacketToSend>> primary_packets =
        fec_generator_primary_->GetFecPackets();
    fec_packets.insert(fec_packets.end(),
                       std::make_move_iterator(primary_packets.begin()),
                       std::make_move_iterator(primary_packets.end()));
  }
  if (fec_generator_secondary_) {
    std::vector<std::unique_ptr<RtpPacketToSend>> secondary_packets =
        fec_generator_secondary_->GetFecPackets();
    fec_packets.insert(fec_packets.end(),
                       std::make_move_iterator(secondary_packets.begin()),
                       std::make_move_iterator(secondary_packets.end()));
  }
  return fec_packets;
}

RtpSendRates RtpSenderEgress::GetSendRatesForPathHint(Timestamp now,
                                                       int path_hint) const {
  RTC_DCHECK_RUN_ON(worker_queue_);
  const std::vector<BitrateTracker>& path_rates =
      NormalizePathHint(path_hint) == RtpPacketToSend::kSecondaryPathHint
          ? send_rates_secondary_
          : send_rates_primary_;
  RtpSendRates send_rates;
  for (size_t i = 0; i < kNumMediaTypes; ++i) {
    const RtpPacketMediaType type = static_cast<RtpPacketMediaType>(i);
    send_rates[type] = path_rates[i].Rate(now).value_or(DataRate::Zero());
  }
  return send_rates;
}
...
void RtpSenderEgress::UpdateRtpStats(Timestamp now,
  ...
  std::vector<BitrateTracker>& per_path_rates =
      NormalizePathHint(path_hint) == RtpPacketToSend::kSecondaryPathHint
          ? send_rates_secondary_
          : send_rates_primary_;
  per_path_rates[static_cast<size_t>(packet_type)].Update(packet_size, now);
  if (bitrate_callback_) {
...
VideoFecGenerator* RtpSenderEgress::SelectFecGeneratorForPacket(
  ...
  if (!fec_generator_secondary_ ||
      packet.packet_type() != RtpPacketMediaType::kVideo) {
    return fec_generator_primary_;
  }
  if (!current_fec_frame_timestamp_.has_value() ||
      *current_fec_frame_timestamp_ != packet.Timestamp() ||
      current_fec_frame_generator_ == nullptr) {
    current_fec_frame_timestamp_ = packet.Timestamp();
    if (NormalizePathHint(packet.path_hint()) ==
        RtpPacketToSend::kSecondaryPathHint) {
      current_fec_frame_generator_ = fec_generator_secondary_;
    } else {
      current_fec_frame_generator_ = fec_generator_primary_;
    }
  }
  VideoFecGenerator* selected = current_fec_frame_generator_;
  if (packet.Marker()) {
    current_fec_frame_timestamp_.reset();
    current_fec_frame_generator_ = nullptr;
  }
  return selected;
}

int RtpSenderEgress::NormalizePathHint(int path_hint) {
  return path_hint == RtpPacketToSend::kSecondaryPathHint
             ? RtpPacketToSend::kSecondaryPathHint
