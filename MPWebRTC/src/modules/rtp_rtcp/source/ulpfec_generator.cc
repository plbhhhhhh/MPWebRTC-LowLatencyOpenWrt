...
std::vector<std::unique_ptr<RtpPacketToSend>> UlpfecGenerator::GetFecPackets() {
  ...
  for (const auto* fec_packet : generated_fec_packets_) {
    ...
    red_packet->set_path_hint(RtpPacketToSend::kNoPathHint);
...
std::unique_ptr<VideoFecGenerator> UlpfecGenerator::CloneForPathHint() const {
  return std::make_unique<UlpfecGenerator>(env_, red_payload_type_,
