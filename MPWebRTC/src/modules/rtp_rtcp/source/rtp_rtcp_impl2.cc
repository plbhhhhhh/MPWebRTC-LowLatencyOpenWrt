...
void ModuleRtpRtcpImpl2::SetFecProtectionParamsForPathHint(
    int path_hint,
    const FecProtectionParams& delta_params,
    const FecProtectionParams& key_params) {
  RTC_DCHECK(rtp_sender_);
  rtp_sender_->packet_sender.SetFecProtectionParametersForPathHint(
      path_hint, delta_params, key_params);
}
...
RtpSendRates ModuleRtpRtcpImpl2::GetSendRatesByPathHint(int path_hint) const {
  RTC_DCHECK_RUN_ON(&rtp_sender_->sequencing_checker);
  return rtp_sender_->packet_sender.GetSendRatesForPathHint(
      env_.clock().CurrentTime(), path_hint);
}
