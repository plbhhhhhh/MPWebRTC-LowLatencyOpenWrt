...
class RtpRtcpInterface : public RtcpFeedbackSenderInterface {
 public:
  ...
  // Returns bitrate sent (post-pacing) per packet type for one path hint.
  // `path_hint` follows RtpPacketToSend::*PathHint constants.
  virtual RtpSendRates GetSendRatesByPathHint(int path_hint) const = 0;
  ...
  // Path-scoped variant of SetFecProtectionParams.
  virtual void SetFecProtectionParamsForPathHint(
      int path_hint,
      const FecProtectionParams& delta_params,
      const FecProtectionParams& key_params) = 0;
