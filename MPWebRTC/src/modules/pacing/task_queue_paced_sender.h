...
class TaskQueuePacedSender : public RtpPacketPacer, public RtpPacketSender {
 ...
 private:
  ...
  bool ShouldPreferBestPath(const RtpPacketToSend& packet) const;
  int SelectBestPathHint() const RTC_RUN_ON(task_queue_);
  int SelectPathHintByGccRatio() RTC_RUN_ON(task_queue_);
  void ObservePathHintAssignment(int path_hint) RTC_RUN_ON(task_queue_);
  void AssignPathHintForPacket(RtpPacketToSend& packet) RTC_RUN_ON(task_queue_);
  // Frame-level path affinity cache. Video media packets sharing the same
  // {ssrc, rtp_timestamp} are forced onto the same path to avoid receiver-side
  // reordering that otherwise triggers spurious NACK/RTX.
  static constexpr size_t kFramePathHintCacheSize = 128;
  struct FramePathHintEntry {
    uint32_t ssrc = 0;
    uint32_t rtp_timestamp = 0;
    int path_hint = -1;
    bool valid = false;
  };
  std::optional<int> LookupFramePathHint(uint32_t ssrc, uint32_t rtp_timestamp)
      const RTC_RUN_ON(task_queue_);
  void RememberFramePathHint(uint32_t ssrc,
                             uint32_t rtp_timestamp,
                             int path_hint) RTC_RUN_ON(task_queue_);
  void ResetFramePathHintCache() RTC_RUN_ON(task_queue_);
  ...
  int64_t path_hint_primary_weight_acc_ RTC_GUARDED_BY(task_queue_) = 0;
  int64_t path_hint_secondary_weight_acc_ RTC_GUARDED_BY(task_queue_) = 0;
  int64_t recent_primary_path_load_ RTC_GUARDED_BY(task_queue_) = 0;
  int64_t recent_secondary_path_load_ RTC_GUARDED_BY(task_queue_) = 0;
  // Ring-buffer LRU for frame-level path affinity (video media packets only).
  std::array<FramePathHintEntry, kFramePathHintCacheSize>
      frame_path_hint_cache_ RTC_GUARDED_BY(task_queue_) = {};
  size_t frame_path_hint_cache_next_idx_ RTC_GUARDED_BY(task_queue_) = 0;
