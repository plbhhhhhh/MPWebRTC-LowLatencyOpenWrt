...
constexpr bool kForceMinFecRateForTesting = false;
constexpr int kForcedMinFecRate = 80;
constexpr int kMultipathDeltaFecRateCap = 48;
constexpr int kMultipathKeyFecRateCap = 80;
constexpr float kMultipathLossScale = 0.35f;
...
uint8_t DampenLossForMultipath(uint8_t raw_loss) {
  if (!IsExtendedMultipathMode()) {
    return raw_loss;
  }
  return saturated_cast<uint8_t>(raw_loss * kMultipathLossScale);
}

void RelaxFecProtectionForMultipath(FecProtectionParams* delta_params,
                                    FecProtectionParams* key_params) {
  if (!IsExtendedMultipathMode()) {
    return;
  }
  delta_params->fec_rate =
      std::min(delta_params->fec_rate, kMultipathDeltaFecRateCap);
  key_params->fec_rate = std::min(key_params->fec_rate, kMultipathKeyFecRateCap);
  // Allow spanning more frames when playout can wait, reducing packet-level
  // redundancy bursts in multipath.
  delta_params->max_fec_frames = std::max(delta_params->max_fec_frames, 3);
  key_params->max_fec_frames = std::max(key_params->max_fec_frames, 3);
}
...
std::unique_ptr<VideoFecGenerator> MaybeCreateFecGenerator(
  ...
  if (rtp.flexfec.payload_type >= 0) {
    ...
    RTC_LOG(LS_INFO) << "FEC setup: creating FlexFEC generator for ssrc="
                     << rtp.ssrcs[simulcast_index]
                     << ", flexfec_pt=" << rtp.flexfec.payload_type;
    ...
    RTC_LOG(LS_INFO) << "FEC setup: creating ULPFEC generator for ssrc="
                     << rtp.ssrcs[simulcast_index]
                     << ", red_pt=" << rtp.ulpfec.red_payload_type
                     << ", ulpfec_pt=" << rtp.ulpfec.ulpfec_payload_type;
  ...
  RTC_LOG(LS_INFO) << "FEC setup: no FEC generator for ssrc="
                   << rtp.ssrcs[simulcast_index]
                   << ", red_pt=" << rtp.ulpfec.red_payload_type
                   << ", ulpfec_pt=" << rtp.ulpfec.ulpfec_payload_type
                   << ", flexfec_pt=" << rtp.flexfec.payload_type;
...
class RtpVideoSender::PathProtectionCallback : public VCMProtectionCallback {
 public:
  PathProtectionCallback(RtpVideoSender* sender, int path_hint)
      : sender_(sender), path_hint_(path_hint) {}
  ...
  int ProtectionRequest(const FecProtectionParams* delta_params,
    ...
    return sender_->ProtectionRequestForPathHint(path_hint_, delta_params,
                                                 key_params,
                                                 sent_video_rate_bps,
                                                 sent_nack_rate_bps,
                                                 sent_fec_rate_bps);
  ...
  void SetRetransmissionMode(int retransmission_mode) override {
    sender_->SetRetransmissionMode(retransmission_mode);
  }

 private:
  RtpVideoSender* const sender_;
  const int path_hint_;
};
...
RtpVideoSender::RtpVideoSender(
  ...
  for (int path_hint : {RtpPacketToSend::kPrimaryPathHint,
                        RtpPacketToSend::kSecondaryPathHint}) {
    const int index = NormalizePathHint(path_hint);
    path_fec_controllers_[index].controller =
        std::make_unique<FecControllerDefault>(env_);
    path_fec_controllers_[index].callback =
        std::make_unique<PathProtectionCallback>(this, path_hint);
    path_fec_controllers_[index].controller->SetProtectionMethod(fec_enabled,
                                                                 NackEnabled());
    path_fec_controllers_[index].controller->SetProtectionCallback(
        path_fec_controllers_[index].callback.get());
  }
...
EncodedImageCallback::Result RtpVideoSender::OnEncodedImage(
  ...
  MutexLock lock(&mutex_);
  const size_t encoded_size = encoded_image.size();
  size_t primary_size = static_cast<size_t>(
      encoded_size * path_fec_controllers_[NormalizePathHint(
                         RtpPacketToSend::kPrimaryPathHint)]
                         .bitrate_share);
  primary_size = std::min(primary_size, encoded_size);
  const size_t secondary_size = encoded_size - primary_size;
  if (path_fec_controllers_[0].controller) {
    path_fec_controllers_[0].controller->UpdateWithEncodedData(
        primary_size, encoded_image._frameType);
  }
  if (path_fec_controllers_[1].controller) {
    path_fec_controllers_[1].controller->UpdateWithEncodedData(
        secondary_size, encoded_image._frameType);
  }
...
void RtpVideoSender::OnBitrateUpdated(BitrateAllocationUpdate update,
  ...
  double primary_share = path_fec_controllers_[0].bitrate_share;
  double secondary_share = path_fec_controllers_[1].bitrate_share;
  const uint64_t recent_primary_rate =
      static_cast<uint64_t>(path_fec_controllers_[0].sent_video_rate_bps) +
      path_fec_controllers_[0].sent_nack_rate_bps +
      path_fec_controllers_[0].sent_fec_rate_bps;
  const uint64_t recent_secondary_rate =
      static_cast<uint64_t>(path_fec_controllers_[1].sent_video_rate_bps) +
      path_fec_controllers_[1].sent_nack_rate_bps +
      path_fec_controllers_[1].sent_fec_rate_bps;
  if (recent_primary_rate + recent_secondary_rate > 0) {
    primary_share = static_cast<double>(recent_primary_rate) /
                    static_cast<double>(recent_primary_rate +
                                        recent_secondary_rate);
    secondary_share = 1.0 - primary_share;
  } else {
    primary_share = 0.5;
    secondary_share = 0.5;
  }
  path_fec_controllers_[0].bitrate_share = primary_share;
  path_fec_controllers_[1].bitrate_share = secondary_share;

  const auto compute_path_loss = [&](int index) -> uint8_t {
    uint8_t raw_loss = 0;
    if (path_fec_controllers_[index].feedback_packets == 0) {
      raw_loss = saturated_cast<uint8_t>(update.packet_loss_ratio * 256);
    } else {
      const double loss_ratio = static_cast<double>(
                                    path_fec_controllers_[index].feedback_lost) /
                                static_cast<double>(
                                    path_fec_controllers_[index].feedback_packets);
      raw_loss = saturated_cast<uint8_t>(loss_ratio * 256.0);
    }
    return DampenLossForMultipath(raw_loss);
  };

  const uint32_t primary_payload_bitrate_bps = saturated_cast<uint32_t>(
      static_cast<double>(payload_bitrate_bps) * primary_share);
  const uint32_t secondary_payload_bitrate_bps =
      payload_bitrate_bps - primary_payload_bitrate_bps;

  if (path_fec_controllers_[0].controller) {
    path_fec_controllers_[0].controller->UpdateFecRates(
        primary_payload_bitrate_bps, framerate, compute_path_loss(0),
        path_fec_controllers_[0].loss_mask_vector, update.round_trip_time.ms());
    path_fec_controllers_[0].loss_mask_vector.clear();
    path_fec_controllers_[0].feedback_packets = 0;
    path_fec_controllers_[0].feedback_lost = 0;
  }
  if (path_fec_controllers_[1].controller) {
    path_fec_controllers_[1].controller->UpdateFecRates(
        secondary_payload_bitrate_bps, framerate, compute_path_loss(1),
        path_fec_controllers_[1].loss_mask_vector, update.round_trip_time.ms());
    path_fec_controllers_[1].loss_mask_vector.clear();
    path_fec_controllers_[1].feedback_packets = 0;
    path_fec_controllers_[1].feedback_lost = 0;
  }
...
int RtpVideoSender::ProtectionRequest(const FecProtectionParams* delta_params,
  ...
  *sent_video_rate_bps =
      path_fec_controllers_[0].sent_video_rate_bps +
      path_fec_controllers_[1].sent_video_rate_bps;
  *sent_nack_rate_bps =
      path_fec_controllers_[0].sent_nack_rate_bps +
      path_fec_controllers_[1].sent_nack_rate_bps;
  *sent_fec_rate_bps = path_fec_controllers_[0].sent_fec_rate_bps +
                       path_fec_controllers_[1].sent_fec_rate_bps;

  // Fallback: before per-path controllers are initialized, apply global params.
  if (!path_fec_controllers_[0].controller || !path_fec_controllers_[1].controller) {
    ProtectionRequestForPathHint(RtpPacketToSend::kPrimaryPathHint, delta_params,
                                 key_params, sent_video_rate_bps,
                                 sent_nack_rate_bps, sent_fec_rate_bps);
    return 0;
  }

  RTC_LOG(LS_INFO) << "FEC setup: aggregated sent_video/nack/fec="
                   << *sent_video_rate_bps << "/" << *sent_nack_rate_bps
                   << "/" << *sent_fec_rate_bps
                   << ", path0 rate=" << path_fec_controllers_[0].delta_params.fec_rate
                   << ", path1 rate="
                   << path_fec_controllers_[1].delta_params.fec_rate
                   << ", global req delta/key=" << delta_params->fec_rate << "/"
                   << key_params->fec_rate;
  return 0;
}

int RtpVideoSender::ProtectionRequestForPathHint(
    int path_hint,
    ...
    uint32_t* sent_fec_rate_bps) {
  const int index = NormalizePathHint(path_hint);
  FecProtectionParams effective_delta_params = *delta_params;
  FecProtectionParams effective_key_params = *key_params;
  if (kForceMinFecRateForTesting) {
    effective_delta_params.fec_rate =
        std::max(effective_delta_params.fec_rate, kForcedMinFecRate);
    effective_key_params.fec_rate =
        std::max(effective_key_params.fec_rate, kForcedMinFecRate);
    effective_delta_params.max_fec_frames =
        std::max(effective_delta_params.max_fec_frames, 1);
    effective_key_params.max_fec_frames =
        std::max(effective_key_params.max_fec_frames, 1);
  }
  RelaxFecProtectionForMultipath(&effective_delta_params,
                                 &effective_key_params);

  path_fec_controllers_[index].delta_params = effective_delta_params;
  path_fec_controllers_[index].key_params = effective_key_params;

  *sent_video_rate_bps = 0;
  *sent_nack_rate_bps = 0;
  *sent_fec_rate_bps = 0;
  for (const RtpStreamSender& stream : rtp_streams_) {
    stream.rtp_rtcp->SetFecProtectionParamsForPathHint(
        path_hint, effective_delta_params, effective_key_params);
    auto send_bitrate = stream.rtp_rtcp->GetSendRatesByPathHint(path_hint);
  ...
  path_fec_controllers_[index].sent_video_rate_bps = *sent_video_rate_bps;
  path_fec_controllers_[index].sent_nack_rate_bps = *sent_nack_rate_bps;
  path_fec_controllers_[index].sent_fec_rate_bps = *sent_fec_rate_bps;
  return 0;
}

int RtpVideoSender::NormalizePathHint(int path_hint) {
  return path_hint == RtpPacketToSend::kSecondaryPathHint ? 1 : 0;
}
...
void RtpVideoSender::OnPacketFeedbackVector(
  ...
  const bool multipath_send_active = IsExtendedMultipathMode();
  {
    MutexLock lock(&mutex_);
    if (fec_controller_->UseLossVectorMask() && !multipath_send_active) {
      for (const StreamPacketInfo& packet : packet_feedback_vector) {
        loss_mask_vector_.push_back(!packet.received);
      }
    }
    for (const StreamPacketInfo& packet : packet_feedback_vector) {
      const int path_index = NormalizePathHint(packet.path_hint);
      if (!multipath_send_active) {
        path_fec_controllers_[path_index].feedback_packets++;
        if (!packet.received) {
          path_fec_controllers_[path_index].feedback_lost++;
        }
      } else if (packet.received) {
        // In dual-path modes, packet-level "not received" in TWCC frequently
        // represents temporary reordering (late ACK) rather than true loss.
        // Track only positively received packets for per-path FEC statistics.
        path_fec_controllers_[path_index].feedback_packets++;
      }
      if (!multipath_send_active &&
          path_fec_controllers_[path_index].controller &&
          path_fec_controllers_[path_index].controller->UseLossVectorMask()) {
        path_fec_controllers_[path_index].loss_mask_vector.push_back(
            !packet.received);
      }
    }
  }
  ...
  // In dual-path send modes (extended/round-robin), reordering is expected.
  // Early-loss immediate resend tends to overreact to temporary out-of-order
  // arrival and inflates RTX/FEC overhead. Keep ACK cleanup, but suppress this
  // immediate resend path and rely on normal NACK/RTX timing.
  if (IsExtendedMultipathMode()) {
    for (const auto& kv : acked_packets_per_ssrc) {
      ...
      if (it == ssrc_to_rtp_module_.end()) {
        // No media, likely FEC or padding. Ignore since there's no RTP history
        // to clean up anyway.
        continue;
      }
...
void RtpVideoSender::SetEncodingData(size_t width,
  ...
  for (auto& path_state : path_fec_controllers_) {
    if (path_state.controller) {
      path_state.controller->SetEncodingData(width, height, num_temporal_layers,
                                             rtp_config_.max_packet_size);
    }
  }
}
