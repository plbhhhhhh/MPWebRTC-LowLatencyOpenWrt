...
class RtpVideoSender : public RtpVideoSenderInterface,
 ...
 private:
  class PathProtectionCallback;
  struct PathFecControllerState {
    std::unique_ptr<FecController> controller;
    std::unique_ptr<PathProtectionCallback> callback;
    ...
    uint64_t feedback_lost = 0;
    double bitrate_share = 0.5;
  };
  ...
  int ProtectionRequestForPathHint(
      int path_hint,
  ...
  std::array<PathFecControllerState, 2> path_fec_controllers_;
