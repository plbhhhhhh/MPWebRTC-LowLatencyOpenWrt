...
BasicIceController::SortAndSwitchConnection(IceSwitchReason reason) {
  ...
  // [新增] 列表刚被重排，此时 connections_[0] 是最好的。
  // 我们利用这个有序列表来寻找第二好的。
  UpdateSecondBestConnection();
...
// 4. [新增函数实现] UpdateSecondBestConnection
// 逻辑：在有序列表中，跳过主连接，优先找异构网络，其次找同构网络。
// [p2p/base/basic_ice_controller.cc]

void BasicIceController::UpdateSecondBestConnection() {
  second_selected_connection_ = nullptr;
  
  // 如果连主连接都没有，或者列表太短，就没有备选的必要
  if (connections_.empty() || !selected_connection_) {
    return;
  }

  const Connection* best_heterogeneous = nullptr; // 异构网络（不同网卡）
  const Connection* best_homogeneous = nullptr;   // 同构网络（同网卡）

  // 获取主连接的 network_id
  // [Fix] 使用 local_candidate().network_id() 而不是 network()->network_id()
  uint16_t primary_network_id = selected_connection_->local_candidate().network_id();

  // 遍历排好序的列表 (Index 0 是当前最优，往后越来越差)
  for (const Connection* conn : connections_) {
    // 跳过主连接自己
    ...
    // 必须是可写的 (Writable)，否则不能作为即时备选
    if (!conn->writable()) {
      continue;
    }

    // [Fix] 修正后的异构判断逻辑
    if (conn->local_candidate().network_id() != primary_network_id) {
      // 找到了排序最靠前的异构连接，直接命中！
      best_heterogeneous = conn;
      break; // 列表有序，找到的第一个肯定就是最好的异构连接
    } else {
      // 记录遇到的第一个（最好的）同构连接作为保底
      if (!best_homogeneous) {
        best_homogeneous = conn;
      }
    }
  }

  // 决策优先：异构 > 同构
  if (best_heterogeneous) {
    second_selected_connection_ = best_heterogeneous;
  } else {
    second_selected_connection_ = best_homogeneous;
  }
  
  if (second_selected_connection_) {
     RTC_LOG(LS_INFO) << "BasicIceController selected secondary connection: " 
                         << second_selected_connection_->ToString();
  }
}
