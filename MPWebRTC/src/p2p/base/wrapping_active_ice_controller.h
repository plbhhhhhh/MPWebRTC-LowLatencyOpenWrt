...
class WrappingActiveIceController : public ActiveIceControllerInterface {
 public:
  ...
  // [新增] 透传调用给内部的 wrapped_ 控制器
  const Connection* GetSecondBestConnection() const override RTC_RUN_ON(network_thread_) {
    return wrapped_ ? wrapped_->GetSecondBestConnection() : nullptr;
  }
