...
class RTC_EXPORT P2PTransportChannel : public IceTransportInternal,
 ...
 private:
  ...
  // [新增] 次优连接 (Secondary) - 用于多路径传输的本地缓存
  Connection* second_selected_connection_ RTC_GUARDED_BY(network_thread_) = nullptr;

  // [新增] 辅助函数：从 Controller 同步次优连接
  void UpdateSecondSelectedConnection();
  ...
  std::array<uint64_t, 5> priority_packets_in_log_window_ = {};
  uint64_t packets_received_ = 0;
  // Multipath scheduler state (window-based quota in bytes/packets).
  int64_t sched_window_start_ms_ = 0;
  int sched_window_duration_ms_ = 0;
  uint64_t primary_budget_bytes_remaining_ = 0;
  uint64_t secondary_budget_bytes_remaining_ = 0;
  uint32_t primary_budget_packets_remaining_ = 0;
  uint32_t secondary_budget_packets_remaining_ = 0;
  uint64_t primary_non_vip_budget_bytes_remaining_ = 0;
  uint32_t primary_non_vip_budget_packets_remaining_ = 0;
  uint32_t primary_estimated_bps_ = 0;
  uint32_t secondary_estimated_bps_ = 0;
  bool frame_interval_last_frame_valid_ = false;
  uint32_t frame_interval_last_frame_ssrc_ = 0;
  uint32_t frame_interval_last_frame_timestamp_ = 0;
  int64_t frame_interval_last_frame_send_ms_ = 0;
  int64_t frame_interval_ewma_ms_ = 0;
  int frame_interval_increase_streak_ = 0;
  int64_t last_frame_interval_sort_request_ms_ = 0;
  uint64_t sched_window_count_ = 0;
  uint64_t batches_in_current_window_ = 0;
  uint64_t primary_batch_quota_bytes_ = 0;
  uint64_t secondary_batch_quota_bytes_ = 0;
  uint64_t primary_batch_sent_bytes_ = 0;
  uint64_t secondary_batch_sent_bytes_ = 0;
  uint32_t primary_over_quota_packets_in_window_ = 0;
  uint32_t secondary_over_quota_packets_in_window_ = 0;
  bool primary_bitrate_from_probe_ = false;
  bool secondary_bitrate_from_probe_ = false;
  size_t sched_avg_packet_size_bytes_ = 1200;
  uint64_t primary_probe_packets_sent_ = 0;
  uint64_t secondary_probe_packets_sent_ = 0;
  int64_t other_primary_current_weight_ = 0;
  int64_t other_secondary_current_weight_ = 0;
  uint16_t last_primary_network_id_for_probe_ = 0;
  uint16_t last_secondary_network_id_for_probe_ = 0;
  int64_t last_periodic_probe_request_ms_ = 0;
  bool wait_probe_for_primary_path_ = false;
  bool wait_probe_for_secondary_path_ = false;

  // For `WebrtcMultipathSendMode::kRoundRobin`: alternates preferred path per
  // sender frame (network_thread_).
  uint32_t multipath_round_robin_phase_ RTC_GUARDED_BY(network_thread_) = 0;
  bool multipath_round_robin_frame_valid_ RTC_GUARDED_BY(network_thread_) =
      false;
  uint32_t multipath_round_robin_frame_ssrc_ RTC_GUARDED_BY(network_thread_) =
      0;
  uint32_t multipath_round_robin_frame_timestamp_
      RTC_GUARDED_BY(network_thread_) = 0;
