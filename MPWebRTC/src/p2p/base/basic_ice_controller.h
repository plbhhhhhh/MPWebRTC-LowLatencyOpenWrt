...
class BasicIceController : public IceControllerInterface {
 public:
  ...
  const Connection* GetSecondBestConnection() const override {
    return second_selected_connection_;
  }
 ...
 private:
  ...
  void UpdateSecondBestConnection();
  ...
  // [新增] 维护次优连接
  const Connection* second_selected_connection_ = nullptr;
