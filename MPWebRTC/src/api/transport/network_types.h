...
struct RTC_EXPORT PacketResult {
  ...
  struct RtpPacketInfo {
    ...
    // Path hint assigned by pacer/scheduler, -1 means "unspecified".
    int path_hint = -1;
  };
  ...
  // Local NetworkRoute id when the packet was sent (multipath send). Absent if
  // unknown; GCC may partition feedback per path using this field.
  std::optional<uint16_t> send_path_network_id;
