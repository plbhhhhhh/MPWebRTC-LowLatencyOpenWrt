...
void VideoReceiveStream2::UpdatePlayoutDelays() const {
  RTC_DCHECK_RUN_ON(&worker_sequence_checker_);
  const bool enforce_base_minimum_delay_floor =
      GetWebrtcMultipathSendMode() == WebrtcMultipathSendMode::kExtended &&
      base_minimum_playout_delay_.has_value();
  ...
  if (frame_maximum_playout_delay_.has_value() &&
    ...
    const bool frame_maximum_delay_violates_multipath_floor =
        enforce_base_minimum_delay_floor &&
        *frame_maximum_playout_delay_ < *base_minimum_playout_delay_;
    if (enforce_base_minimum_delay_floor &&
        *frame_maximum_playout_delay_ < *base_minimum_playout_delay_) {
      // Keep the currently computed minimum delay. We only ignore frame max as
      // a limiter if it falls below the multipath base minimum floor.
      RTC_LOG(LS_WARNING)
          << "Multipath mode keeps base minimum delay "
          << *base_minimum_playout_delay_
          << " despite lower frame maximum delay "
          << *frame_maximum_playout_delay_;
    } else {
      minimum_delay = *frame_maximum_playout_delay_;
    }
    if (timing_->min_playout_delay() != *minimum_delay &&
        !frame_maximum_delay_violates_multipath_floor) {
      RTC_LOG(LS_WARNING)
  ...
  if (frame_maximum_playout_delay_.has_value()) {
    TimeDelta effective_maximum_delay = *frame_maximum_playout_delay_;
    if (enforce_base_minimum_delay_floor &&
        effective_maximum_delay < *base_minimum_playout_delay_) {
      effective_maximum_delay = *base_minimum_playout_delay_;
    }
    if (effective_maximum_delay < *minimum_delay) {
      effective_maximum_delay = *minimum_delay;
    }
    timing_->set_playout_delay({*minimum_delay, effective_maximum_delay});
  } else {
