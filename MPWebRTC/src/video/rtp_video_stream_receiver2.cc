...
constexpr int kPacketBufferStartSizeMultipath = 2048;
constexpr int kPacketBufferMaxSize = 2048;
constexpr int kPacketBufferMaxSizeMultipath = 8192;

constexpr int kMaxPacketAgeToNack = 450;
constexpr int kMaxPacketAgeToNackMultipath = 2200;

bool IsExtendedMultipathMode() {
  return GetWebrtcMultipathSendMode() == WebrtcMultipathSendMode::kExtended;
}

int MaxPacketAgeToNackForCurrentMode() {
  if (IsExtendedMultipathMode()) {
    return kMaxPacketAgeToNackMultipath;
  }
  return kMaxPacketAgeToNack;
}
...
int PacketBufferMaxSize(const FieldTrialsView& field_trials) {
  ...
  if (IsExtendedMultipathMode()) {
    packet_buffer_max_size =
        std::max(packet_buffer_max_size, kPacketBufferMaxSizeMultipath);
  }
  return packet_buffer_max_size;
}

int PacketBufferStartSize() {
  if (IsExtendedMultipathMode()) {
    return kPacketBufferStartSizeMultipath;
  }
  return kPacketBufferStartSize;
}
...
RtpVideoStreamReceiver2::RtpVideoStreamReceiver2(
    ...
    : env_(env),
      ...
      nack_module_(MaybeConstructNackModule(env_,
                                            ...
                                            &rtcp_feedback_buffer_)),
      packet_buffer_(PacketBufferStartSize(),
  ...
  if (config_.rtp.nack.rtp_history_ms > 0) {
    rtp_receive_statistics_->SetMaxReorderingThreshold(config_.rtp.remote_ssrc,
                                                       MaxPacketAgeToNackForCurrentMode());
  }
  ParseFieldTrial(
      {&forced_playout_delay_max_ms_, &forced_playout_delay_min_ms_},
      env_.field_trials().Lookup("WebRTC-ForcePlayoutDelay"));

  if (config_.rtp.lntf.enabled && IsExtendedMultipathMode()) {
    RTC_LOG(LS_WARNING)
        << "Loss notification is disabled in extended multipath mode because it does "
           "not tolerate packet reordering well.";
  } else if (config_.rtp.lntf.enabled) {
...
void RtpVideoStreamReceiver2::SetLossNotificationEnabled(bool enabled) {
  RTC_DCHECK_RUN_ON(&packet_sequence_checker_);
  if (enabled && IsExtendedMultipathMode()) {
    if (loss_notification_controller_) {
      loss_notification_controller_.reset();
      rtcp_feedback_buffer_.ClearLossNotificationState();
    }
    RTC_LOG(LS_WARNING)
        << "Ignoring SetLossNotificationEnabled(true) in extended "
           "multipath mode.";
    return;
  }
...
void RtpVideoStreamReceiver2::SetNackHistory(TimeDelta history) {
  ...
  rtp_receive_statistics_->SetMaxReorderingThreshold(
      config_.rtp.remote_ssrc,
      history.ms() > 0 ? MaxPacketAgeToNackForCurrentMode()
