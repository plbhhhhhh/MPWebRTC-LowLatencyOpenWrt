...
class ScopedMultipathSendModeForTest {
 public:
  explicit ScopedMultipathSendModeForTest(WebrtcMultipathSendMode mode)
      : previous_mode_(GetWebrtcMultipathSendMode()) {
    SetWebrtcMultipathSendMode(mode);
  }

  ~ScopedMultipathSendModeForTest() {
    SetWebrtcMultipathSendMode(previous_mode_);
  }

 private:
  const WebrtcMultipathSendMode previous_mode_;
};
...
TEST_P(VideoReceiveStream2Test,
       MultipathKeepsBaseMinimumDelayWhenFrameMaxIsLower) {
  ScopedMultipathSendModeForTest scoped_mode(WebrtcMultipathSendMode::kExtended);
  const VideoPlayoutDelay kPlayoutDelay(TimeDelta::Millis(0),
                                        TimeDelta::Millis(321));
  video_receive_stream_->OnCompleteFrame(test::FakeFrameBuilder()
                                             .Id(0)
                                             .PlayoutDelay(kPlayoutDelay)
                                             ...
                                             .Build());
  EXPECT_TRUE(video_receive_stream_->SetBaseMinimumPlayoutDelayMs(500));
  EXPECT_EQ(timing_->GetTimings().min_playout_delay, TimeDelta::Millis(500));
  EXPECT_EQ(timing_->GetTimings().max_playout_delay, TimeDelta::Millis(500));
}

TEST_P(VideoReceiveStream2Test,
       NativeModeStillLimitsMinimumDelayByFrameMaxPlayoutDelay) {
  ScopedMultipathSendModeForTest scoped_mode(WebrtcMultipathSendMode::kNative);
  const VideoPlayoutDelay kPlayoutDelay(TimeDelta::Millis(0),
                                        TimeDelta::Millis(321));
  video_receive_stream_->OnCompleteFrame(test::FakeFrameBuilder()
                                             .Id(0)
                                             ...
                                             .Build());
  EXPECT_TRUE(video_receive_stream_->SetBaseMinimumPlayoutDelayMs(500));
  EXPECT_EQ(timing_->GetTimings().min_playout_delay, TimeDelta::Millis(321));
  EXPECT_EQ(timing_->GetTimings().max_playout_delay, TimeDelta::Millis(321));
}
