...
constexpr size_t kMaxFramesBufferedMultipath = 3200;
// Max number of decoded frame info that will be saved.
constexpr int kMaxFramesHistory = 1 << 13;
constexpr int kMaxFramesHistoryMultipath = 1 << 15;
...
size_t MaxFramesBufferedForCurrentMode() {
  if (GetWebrtcMultipathSendMode() == WebrtcMultipathSendMode::kExtended) {
    return kMaxFramesBufferedMultipath;
  }
  return kMaxFramesBuffered;
}

int MaxFramesHistoryForCurrentMode() {
  if (GetWebrtcMultipathSendMode() == WebrtcMultipathSendMode::kExtended) {
    return kMaxFramesHistoryMultipath;
  }
  return kMaxFramesHistory;
}
...
VideoStreamBufferController::VideoStreamBufferController(
    ...
    : field_trials_(field_trials),
      ...
      buffer_(std::make_unique<FrameBuffer>(MaxFramesBufferedForCurrentMode(),
                                            MaxFramesHistoryForCurrentMode(),
                                            field_trials)),
